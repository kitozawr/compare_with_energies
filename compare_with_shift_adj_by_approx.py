...
def fit_and_check_shift(fit_function, energies_parrots, maxima_new, best_shift, max_r_2, num_matched, bounds = (np.array([1e-5, -np.inf]), np.array([np.inf, np.inf]))):
	popt, pcov = curve_fit(fit_function, energies_parrots, maxima_new, bounds=bounds)

	a = popt[0]
	b = popt[1]
	cur_r_2 = r_2(energies_parrots, maxima_new, a, b)
	print("a = {}, b = {}, r_2 = {}".format(a,b,cur_r_2))
...
parser.add_argument('-ro','--rem_out', dest='remove_outliers', help="remove points that outlines trend more than N_out sigma values.", action="store_true")
...
if rest_argv != []:
	...
	if ext == '.dat' or ext == '.png':
		...
		print("Error: unknown ext!")
		raise TypeError('Unknown extension')
	if 'old_osc' in rest_argv:
		old_osc = True
		print("Данные считываются, как для старой версии осциллоскопа.")
	else:
		old_osc = False
		print("Данные считываются, как для новой версии осциллоскопа.")


	if 'exact' in rest_argv:
		try_no_losses = False
	elif 'try' in rest_argv:
		try_no_losses = True
	else:
		try_no_losses = False
	if 'same' in rest_argv:
		same_computer = True
	else:
		same_computer = False
	if 'inv' in rest_argv:
	...
	if 'aflr2' in rest_argv: #aflr2 = autofind low r2 - run shift search in case of r2 lower than low_r2_threshold.
		aflr2 = True
		print("Задан параметр aflr2. В случае, если найденный оптимальный сдвиг окажется меньше low_r2_threshold={}, будет сделана попытка найти оптимальный shift автоматически.".format(low_r2_threshold))
	else:
		aflr2 = False
	...
	for foldername_ac in foldernames_ac:
		...
		if filename_en_splitted == foldername_ac_splitted:
			...
			if 'kalibr' in foldername_ac_splitted or 'calibr' in foldername_ac_splitted:
				maxima = dp.read_maxima(foldername_ac, filenames_ac_times, filenames_ac_info, ext, area=area, fon_coeff=1.0, old_osc=args.old_osc, limit_max = True, inv=True)
			else:
				maxima = dp.read_maxima(foldername_ac, filenames_ac_times, filenames_ac_info, ext, area=area, fon_coeff=1.0, old_osc=args.old_osc, limit_max = False, inv=args.inv)
			...
			if ft == 'ac' and args.same_computer:
				...
				#########NEW#############
				if args.rem_out:
					best_shift = -1; max_r_2 = -1; num_matched = 0; a = 0; b = 0
					best_shift, max_r2, a, b, num_matched = fit_and_check_shift(fit_function, energies_parrots, maxima_new, best_shift, max_r_2, num_matched, bounds = (np.array([1e-5, -np.inf]), np.array([np.inf, np.inf])))
					...
					energies_parrots_rem_out = energies_parrots_rem_out[abs(energies_parrots_rem_out > is_zero)]
					maxima_new_rem_out = maxima_new_rem_out[abs(maxima_new_rem_out > is_zero)]
				...
				for shift in range(shift_min, shift_max+1):
					...
					if args.try_no_losses == True and dp.check_if_there_were_lost(filename_en, foldername_ac, ext, col_en=col_en, col_fon=col_fon, col_trig=col_trig, use_fon = use_fon, old_osc=args.old_osc):
						...
						if energies_parrots_old.size <= 1:
							...
							continue
						if args.inv and 'kalibr' not in foldername_ac_splitted and 'calibr' not in foldername_ac_splitted:
							best_shift_old, max_r_2_old, a_old, b_old, num_matched_old = fit_and_check_shift(fit_function, energies_parrots_old, maxima_new_old, best_shift_old, max_r_2_old, num_matched_old, bounds=bounds_inv)
						else:
							best_shift_old, max_r_2_old, a_old, b_old, num_matched_old = fit_and_check_shift(fit_function, energies_parrots_old, maxima_new_old, best_shift_old, max_r_2_old, num_matched_old, bounds=bounds_not_inv)
					else:
					...
					if args.inv and 'kalibr' not in foldername_ac_splitted and 'calibr' not in foldername_ac_splitted:
						best_shift, max_r_2, a, b, num_matched = fit_and_check_shift(fit_function, energies_parrots, maxima_new, best_shift, max_r_2, num_matched_old, bounds=bounds_inv)
					else:
						best_shift, max_r_2, a, b, num_matched = fit_and_check_shift(fit_function, energies_parrots, maxima_new, best_shift, max_r_2, num_matched, bounds=bounds_not_inv)
